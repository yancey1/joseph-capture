...
import slh.capture.service.IZqSkyScrabService;

@Service("zqSkyScrabService")
public class ZqSkyScrabServiceImpl extends ScrabServiceImpl implements IZqSkyScrabService {
  ...
  public List<ScrabData> execute(CloseableHttpClient client, String month, String imgCode) {
    ...
    form.setCompany(CompanyEnum.ZQSKY.getCompanyName());
    ...
    for (CapturePlat capturePlat : capturePlats) {
      ...
      for (String string : urlList) {
        ...
        for (ScrabData scrabData : list) {
          if(scrabData.getBizDate().equals("2014-08-25")){
            System.out.println(scrabData);
          }
